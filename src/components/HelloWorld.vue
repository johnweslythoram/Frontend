          ...
          <div class="forms-wrap full-width">
            <form @submit.prevent="submitForgotPassword" autocomplete="off">
              ...
              <div class="actual-form">
                ...
                <input type="submit" value="Update Password" class="sign-btn" />
              </div>
            </form>
          </div>
        </div>
...
//     mounted() {
//   const hash = window.location.hash; // e.g., #access_token=xyz&refresh_token=abc...
...
// },
  ...
  const refreshToken = params.get('refresh_token')
        if(this.password != this.Cpassword){
            swal("Passwords should Match",'error')
            return
        }
        if (token){
            try{
                ...
                if(response.data.message){
                    ...
                    swal(`Hey ${username} You can login now`,'success')
                    this.$router.replace('/')
                }
            }
            catch(err){
                if(err){
                    swal(err.response.data.error,'info')
                }
            }
            finally{
                this.loading = false
